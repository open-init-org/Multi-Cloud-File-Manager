...
class EphemeralWorker (private var mContext: Context, workerParams: WorkerParameters): Worker(mContext, workerParams) {
    ...
    companion object {
        ...
        const val DELETE_CURRENTPATH = "DELETE_CURRENTPATH"
    }
            ...
            if(preconditionsMet()) {
                ...
                when(type){
                    ...
                    Type.UPLOAD -> {
                        ...
                        handleSync(mTitle)
                        target?.let { sendUploadFinishedBroadcast(remoteItem.name, it) }
                    }
                    Type.MOVE -> {
                    ...
                    Type.DELETE -> {
                        ...
                        val currentPath = inputData.getString(DELETE_CURRENTPATH)
                        silentRun = true
                        ...
                        handleSync(mTitle)
                        val broadcastPath = currentPath ?: fileItem.path
                        broadcastPath?.let { sendUploadFinishedBroadcast(remoteItem.name, it) }
                    }
                }
            } else {
