package org.openinit.multicloudfilemanager.Dialogs;
...
public class FilePropertiesDialog extends DialogFragment {
    ...
    private final String SAVED_FOLDER_BYTES = "ca.pkay.rcexplorer.FilePropertiesDialog.FOLDER_BYTES";
    private final String SAVED_FOLDER_COUNT = "ca.pkay.rcexplorer.FilePropertiesDialog.FOLDER_COUNT";
    ...
    // folder size state (-1 = not yet calculated)
    private long folderBytes = -1;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        if (savedInstanceState != null) {
            ...
            folderBytes = savedInstanceState.getLong(SAVED_FOLDER_BYTES, -1);
            folderCount = savedInstanceState.getLong(SAVED_FOLDER_COUNT, -1);
        }

        rclone = new Rclone(context);
        asyncTasks = new AsyncTask[3];
        ...
        if (fileItem.isDir()) {
            ...
            view.findViewById(R.id.folder_size_container).setVisibility(View.GONE);
        }
    ...
    private void setupFolderSizeSection() {
        View container = view.findViewById(R.id.folder_size_container);
        container.setVisibility(View.VISIBLE);

        TextView sizeValue = view.findViewById(R.id.folder_size_value);
        TextView countLabel = view.findViewById(R.id.folder_size_count);
        MaterialButton calculateButton = view.findViewById(R.id.folder_size_calculate_button);

        // Restore previously calculated value (e.g. after rotation)
        if (folderBytes >= 0) {
            sizeValue.setText(Formatter.formatFileSize(context, folderBytes));
            countLabel.setText(context.getString(R.string.folder_size_count_format, folderCount));
        ...
        calculateButton.setOnClickListener(v -> {
            ...
            sizeValue.setText(R.string.calculating);
            asyncTasks[2] = new CalculateFolderSize().execute();
        });
    }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putLong(SAVED_FOLDER_BYTES, folderBytes);
        outState.putLong(SAVED_FOLDER_COUNT, folderCount);
        if (md5String != null) {
    ...
    @SuppressLint("StaticFieldLeak")
    private class CalculateFolderSize extends AsyncTask<Void, Void, long[]> {

        @Override
        protected long[] doInBackground(Void... voids) {
            return rclone.calculateFolderSize(remote, fileItem);
        }
        ...
        protected void onPostExecute(long[] result) {
            ...
            if (result == null) {
                sizeValue.setText(R.string.folder_size_error);
                calculateButton.setEnabled(true);
                return;
            }

            folderBytes = result[0];
