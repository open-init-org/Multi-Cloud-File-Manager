...
import static org.openinit.multicloudfilemanager.util.ActivityHelper.tryStartActivity;
import static org.openinit.multicloudfilemanager.util.ActivityHelper.tryStartActivityForResult;
import static org.openinit.multicloudfilemanager.util.ActivityHelper.tryStartService;
...
import org.openinit.multicloudfilemanager.Activities.PhotoViewerActivity;
...
import org.openinit.multicloudfilemanager.Dialogs.Dialogs;
import org.openinit.multicloudfilemanager.Dialogs.FilePropertiesDialog;
import org.openinit.multicloudfilemanager.Dialogs.GoToDialog;
import org.openinit.multicloudfilemanager.Dialogs.InputDialog;
import org.openinit.multicloudfilemanager.Dialogs.LinkDialog;
import org.openinit.multicloudfilemanager.Dialogs.LoadingDialog;
import org.openinit.multicloudfilemanager.Dialogs.OpenAsDialog;
import org.openinit.multicloudfilemanager.Dialogs.ServeDialog;
import org.openinit.multicloudfilemanager.Dialogs.SortDialog;
import org.openinit.multicloudfilemanager.FileComparators;
import org.openinit.multicloudfilemanager.FilePicker;
import org.openinit.multicloudfilemanager.Items.DirectoryObject;
...
import org.openinit.multicloudfilemanager.RecyclerViewAdapters.FileExplorerRecyclerViewAdapter;
import org.openinit.multicloudfilemanager.Services.StreamingService;
import org.openinit.multicloudfilemanager.Services.ThumbnailsLoadingService;
import org.openinit.multicloudfilemanager.util.ActivityHelper;
import org.openinit.multicloudfilemanager.util.FLog;
import org.openinit.multicloudfilemanager.util.LargeParcel;
import org.openinit.multicloudfilemanager.workmanager.EphemeralTaskManager;
import org.openinit.multicloudfilemanager.workmanager.SyncManager;
import org.openinit.multicloudfilemanager.ui.BreadcrumbView;
...
public class FileExplorerFragment extends Fragment implements   FileExplorerRecyclerViewAdapter.OnClickListener,
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        currentViewMode = sharedPreferences.getInt(getString(R.string.pref_key_file_explorer_view_mode), FileExplorerRecyclerViewAdapter.VIEW_TYPE_LIST);
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (showThumbnails || currentViewMode == FileExplorerRecyclerViewAdapter.VIEW_TYPE_GRID) {
        ...
        updateLayoutManager(currentViewMode);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        MenuItem menuViewMode = menu.findItem(R.id.action_view_mode);
        if (menuViewMode != null) {
            if (currentViewMode == FileExplorerRecyclerViewAdapter.VIEW_TYPE_GRID) {
                menuViewMode.setIcon(R.drawable.ic_round_list_24);
                menuViewMode.setTitle(R.string.view_mode_list);
            } else {
                menuViewMode.setIcon(R.drawable.ic_view_module);
                menuViewMode.setTitle(R.string.view_mode_gallery);
            }
        }
    ...
    private void updateLayoutManager(int viewMode) {
        currentViewMode = viewMode;
        if (getContext() == null || recyclerView == null) {
            return;
        }
        if (viewMode == FileExplorerRecyclerViewAdapter.VIEW_TYPE_GRID) {
            int displayWidth = getResources().getDisplayMetrics().widthPixels;
            int itemWidth = (int) (120 * getResources().getDisplayMetrics().density);
            int spanCount = Math.max(3, displayWidth / itemWidth);
            GridLayoutManager gridLayoutManager = new GridLayoutManager(getContext(), spanCount);
            recyclerView.setLayoutManager(gridLayoutManager);
        } else {
            recyclerViewLinearLayoutManager = new LinearLayoutManager(getContext());
        ...
        if (getContext() != null) {
            ...
            sharedPreferences.edit().putInt(getString(R.string.pref_key_file_explorer_view_mode), viewMode).apply();
        }
    }

    private void scrollToPositionOffset(int position) {
    ...
    private void toggleViewMode() {
        int newMode = (currentViewMode == FileExplorerRecyclerViewAdapter.VIEW_TYPE_LIST)
                ? FileExplorerRecyclerViewAdapter.VIEW_TYPE_GRID
                : FileExplorerRecyclerViewAdapter.VIEW_TYPE_LIST;
        if (newMode == FileExplorerRecyclerViewAdapter.VIEW_TYPE_GRID && !isThumbnailsServiceRunning) {
            initializeThumbnailParams();
            startThumbnailService();
        }
    ...
    public boolean onBackButtonPressed() {
        ...
        if (!directoryObject.isContentValid(path)) {
            ...
            if (directoryPosition.containsKey(directoryObject.getCurrentPath())) {
                ...
                scrollToPositionOffset(position);
            }
            fetchDirectoryTask = new FetchDirectoryContent(true).execute();
    ...
    private void openPhotoViewer(FileItem initialFile) {
        List<FileItem> allContent = directoryObject.getDirectoryContent();
        ArrayList<FileItem> photoList = new ArrayList<>();
        int initialPosition = 0;

        for (FileItem item : allContent) {
            if (!item.isDir() && item.getMimeType() != null && item.getMimeType().startsWith("image/")) {
                if (item.equals(initialFile) || (item.getPath() != null && item.getPath().equals(initialFile.getPath()))) {
                    initialPosition = photoList.size();
                }
                photoList.add(item);
            }
        }
        ...
        if (photoList.isEmpty()) {
            photoList.add(initialFile);
        }

        Intent intent = new Intent(getContext(), PhotoViewerActivity.class);
        intent.putParcelableArrayListExtra(PhotoViewerActivity.EXTRA_PHOTOS, photoList);
        intent.putExtra(PhotoViewerActivity.EXTRA_INITIAL_POSITION, initialPosition);
        if (thumbnailServerAuth == null || thumbnailServerPort == 0) {
            initializeThumbnailParams();
            startThumbnailService();
        }
        intent.putExtra(PhotoViewerActivity.EXTRA_THUMBNAIL_AUTH, thumbnailServerAuth);
        intent.putExtra(PhotoViewerActivity.EXTRA_THUMBNAIL_PORT, thumbnailServerPort);
