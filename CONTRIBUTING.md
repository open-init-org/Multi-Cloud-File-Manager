# Contributing to Multi Cloud File Manager

We welcome any contribution to Multi Cloud File Manager, and there are multiple ways to contribute:

 - [Reporting a bug](#reporting-a-bug)
 - [Localize Multi Cloud File Manager into your language](#localize-round-sync)
...
A lot of problems are errors in `rclone.conf`. If you have [Termux](https://github.com/termux/termux-app) installed, you can install rclone  with `pkg install rclone`. Then, export your config from Multi Cloud File Manager and select Termux as target. You can then try to check if the error also occurs in Termux. You can also export and transfer your config to a desktop PC and test it there. 
...
## Localize Multi Cloud File Manager
 - Download [strings.xml](app/src/main/res/values/strings.xml) file.
 - Open the `string.xml` file with your favorite text editor.
 - Delete all the `strings` with the attribute **translatable="false"**.
 - Translate `string` values from **en-US (English)** to that language you want to localize Multi Cloud File Manager.
   Here is an example of translating into **bn-BD**

   Default string values **en-US**
   ```sh
   <string name="app_name">Multi Cloud File Manager</string>
   <string name="app_description">Rclone for Android</string>
   <string name="app_short_name">Multi Cloud File Manager</string>
   ```
...
1. Keep your PR small. Small PRs are easier to review, easier to test and as a result can be merged quickly. If this is your first PR to Multi Cloud File Manager, keep it very small.
...
- For anything related to data transfer or accessing files on your cloud storage, please first check if your idea works in rclone. If it does not work there, it will probably also not work in Multi Cloud File Manager.
- Asking yourself what you can do to create this feature.
- The version of Multi Cloud File Manager you are using.
...
> #### What should Multi Cloud File Manager be able to do differently to help with this problem?

Describe how you would solve your problem. This may include additional buttons, options, menus, dialogs, etc. 

This two-step approach allows us to to design general solutions, that work not just for your specific situation, but for the broader Multi Cloud File Manager user base. It also makes it easier for other community to join the discussion and suggest different solutions.

Please keep in mind that Multi Cloud File Manager and rclone are developed by volunteers.
